<x-layouts.app title="Catálogo de Productos - SUMA">
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">
                    Administra los productos registrados en el sistema, sus precios y marcas.
                </p>
            </div>
        ...
        <x-ui.card padding="p-4" class="shadow-sm border-slate-200 bg-white">
            <form method="GET" action="{{ route('products.index') }}" class="grid gap-3 sm:grid-cols-12">
                ...
                {{-- Filtro Marca --}}
                <div class="sm:col-span-6 lg:col-span-3">
                    <select name="brand_id" class="app-input text-xs" onchange="this.form.submit()">
                ...
                @if($search || $categoryId || $brandId)
                    <div class="sm:col-span-12 flex items-center justify-end">
                        <a href="{{ route('products.index') }}" class="inline-flex items-center gap-1 text-xs text-rose-600 hover:underline font-semibold">
                            ✕ Limpiar filtros
                        </a>
                    </div>
                @endif
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-700">
                    <thead class="border-b border-slate-200 bg-slate-50 text-[11px] font-bold uppercase tracking-wider text-slate-500">
                        <tr>
                            <th scope="col" class="px-6 py-3.5">Producto / SKU</th>
                            <th scope="col" class="px-6 py-3.5">Código / Tipo</th>
                    ...
                    <tbody class="divide-y divide-slate-100 font-medium">
                        @forelse ($products as $product)
                            @php
                                ...
                                $primaryBarcode = $stockItem?->barcodes->firstWhere('is_primary', true)?->barcode
                                    ?? $stockItem?->barcodes->first()?->barcode
                                    ?? 'Sin código';

                                $priceAmount = $stockItem?->prices->first()?->amount;
                            @endphp
                            <tr class="transition hover:bg-slate-50/80">
                                {{-- Nombre y SKU --}}
                                <td class="px-6 py-4">
                                    <div class="font-bold text-slate-900">{{ $product->name }}</div>
                                    <div class="text-xs text-slate-400 font-mono">SKU: {{ $product->sku ?? 'N/A' }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <div class="mt-0.5">
                                        @if($product->product_type === 'bulk')
                                            <span class="inline-block rounded bg-amber-50 px-1.5 py-0.5 text-[10px] font-bold text-amber-700 border border-amber-200">
                                                A Granel
                                            </span>
                                        @else
                                            <span class="inline-block rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold text-slate-600">
                                                Pieza
                                            </span>
                                        @endif
                                    </div>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('products.edit', $product) }}" class="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition" title="Editar producto">
                                        Editar ✏️
                                    </a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-slate-400 font-medium">
                                    No se encontraron productos registrados o que coincidan con los filtros.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
